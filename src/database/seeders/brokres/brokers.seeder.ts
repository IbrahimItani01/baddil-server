    ...
    const brokers = await Promise.all(
      brokerUsers.map(async (user) => {
        ...
        const brokerData = {
          ...
          clients: Array.from({ length: faker.number.int({ min: 1, max: 5 }) }).map(() => ({
            client_id: barterer ? (barterer._id as Types.ObjectId) : new Types.ObjectId(), // Ensure client_id is properly typed as ObjectId
            budget: faker.number.int({ min: 100, max: 10000 }),
            client_goal: {
              name: faker.commerce.productName(),
              condition: faker.helpers.arrayElement(Object.values(ItemConditionEnum)),
            ...
            process: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }).map(() => ({
              process_id: uuidv4(),
          ...
          ratings: Array.from({ length: faker.number.int({ min: 0, max: 5 }) }).map(() => ({
            client_id: barterer ? (barterer._id as Types.ObjectId) : new Types.ObjectId(), // Ensure client_id is properly typed as ObjectId
            rating: faker.helpers.arrayElement(Object.values(RatingEnum)) as RatingEnum,
            message: faker.lorem.sentence(),
            date: faker.date.past(),
          })),
          chats_history: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }).map(() => new Types.ObjectId()),
          barters: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }).map(() => new Types.ObjectId()),
        };
